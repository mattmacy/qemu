...
/*
 * Virtual Structures Table accessors for SBE, IVT, EQDT, VPD
 */
...
static int pnv_xive_get_ive(XiveRouter *xrtr, uint32_t srcno, XiveIVE *ive)
{
    PnvXive *xive = PNV_XIVE(xrtr);
    ...
    /* TODO: check when remote IVE lookups are possible */
    if (pnv_xive_get_ic(xive, blk) != xive) {
        xive_error(xive, "VST: IVE %x is remote !?", srcno);
        return -1;
    }
...
static int pnv_xive_set_ive(XiveRouter *xrtr, uint32_t srcno, XiveIVE *ive)
{
    /* All done. */
    return 0;
}
 ...
 * pages and EQ ESB pages is sub-divided into "sets" which map
...
/*
 * Computes the overall size of the IPI or the EQ ESB pages
 */
...
/*
 * Maps an offset of the VC region in the IPI or EQ region using the
 ...
 * the IPIs and the XiveEQSource object backing the EQs
 */
...
static void pnv_xive_eq_source_realize(PnvXive *xive, Error **errp)
{
    ...
    /* Install the EQ ESB MMIO region in its VC region */
                                         ...
                                         uint8_t blk, uint64_t vsd)
{
    ...
    case VST_TSEL_EQDT:
        /* Same trigger but for the XiveEQSource object backing the EQs. */
        pnv_xive_eq_source_realize(xive, &error_fatal);
        break;

    case VST_TSEL_VPDT:
        /* FIXME (skiboot) : remove DD1 workaround on the VPD table size */
        vst_shift = 16;
        break;
                                  ...
                                  bool mmio)
{
     ...
     * VC BAR containing the ESB pages of the IPIs and of the EQs
     */
...
/*
 * Virtualization Controller MMIO region containing the IPI and EQ ESB pages
 */
                                 ...
                                 unsigned size)
{
    ...
    if (result != MEMTX_OK) {
        xive_error(xive, "VC: %s read failed at @0x%"HWADDR_PRIx " -> @0x%"
                   HWADDR_PRIx, edt_type == CQ_TDR_EDT_IPI ? "IPI" : "EQ",
                   offset, edt_offset);
        return -1;
    }
...
void pnv_xive_pic_print_info(PnvXive *xive, Monitor *mon)
{
    ...
    while (!xive_router_get_ive(xrtr, srcno, &ive)) {
        xive_router_print_ive(xrtr, srcno++, &ive, mon);
    }
...
static void pnv_xive_realize(DeviceState *dev, Error **errp)
{
    ...
    memory_region_init(&xive->eq_mmio, OBJECT(xive), "xive.vc.eq",
                       PNV9_XIVE_VC_SIZE);
    address_space_init(&xive->eq_as, &xive->eq_mmio, "xive.vc.eq");
